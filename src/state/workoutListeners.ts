...
import { completeWorkout, endWorkout, pauseWorkout, resumeWorkout, startWorkout, tickWorkout } from './workoutSlice';
...
export function registerWorkoutListeners(startListening: AppStartListening): void {
  startListening({
  ...
  startListening({
    ...
    effect: async (_, api) => {
      ...
      const elapsedSeconds = Math.floor(state.workout.accumulatedElapsedMs / 1000);
