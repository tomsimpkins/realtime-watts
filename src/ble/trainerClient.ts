...
const SIMULATED_WHEEL_CIRCUMFERENCE_METERS = 2.105;
const SIMULATED_WHEEL_EVENT_UNITS_PER_SECOND = 2048;

type SimulationPacketCallback = (dataView: DataView) => void;
...
class WebBluetoothTrainerClient implements TrainerClient {
	...
	private async startPowerNotifications(): Promise<void> {
		if (!this.powerCharacteristic) {
			throw new Error(
				"Cycling power measurement characteristic is unavailable.",
			);
		}
...
class MockTrainerClient implements TrainerClient {
	private readonly powerCallbacks = new Set<SimulationPacketCallback>();
	private readonly disconnectCallbacks = new Set<DisconnectCallback>();
	...
	private cumulativeWheelRevolutions = 0;
	private wheelEventTime = 0;
	...
	private startEmitter(): void {
		...
		this.intervalId = window.setInterval(() => {
			...
			this.advanceWheelState(speedKph, intervalSeconds);
	...
	private advanceWheelState(speedKph: number, intervalSeconds: number): void {
		const revolutionsPerSecond =
			(speedKph * 1000) / 3600 / SIMULATED_WHEEL_CIRCUMFERENCE_METERS;

		this.wheelRemainder += revolutionsPerSecond * intervalSeconds;
		const completedRevolutions = Math.floor(this.wheelRemainder);

		if (completedRevolutions <= 0) {
			return;
		}

		this.wheelRemainder -= completedRevolutions;
		this.cumulativeWheelRevolutions =
			(this.cumulativeWheelRevolutions + completedRevolutions) % 0x1_0000_0000;

		const ticksPerRevolution = Math.max(
			1,
			Math.round(
				(SIMULATED_WHEEL_CIRCUMFERENCE_METERS *
					3.6 *
					SIMULATED_WHEEL_EVENT_UNITS_PER_SECOND) /
					speedKph,
			),
		);
		this.wheelEventTime =
			(this.wheelEventTime + completedRevolutions * ticksPerRevolution) %
			0x1_0000;
	}
...
function createSimulationPacket(
	...
	view.setUint16(0, (1 << 4) | (1 << 5), true);
	view.setInt16(2, watts, true);
	view.setUint32(4, cumulativeWheelRevolutions, true);
	view.setUint16(8, lastWheelEventTime, true);
	view.setUint16(10, cumulativeCrankRevolutions, true);
	view.setUint16(12, lastCrankEventTime, true);
